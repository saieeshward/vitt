...
| File | Holds |
|---|---|
| `tokens/palette.css` | the fixed six currency hues (light and lifted), destructive, companion colour |
| `tokens/themes.css` | the four themes and three accents, as `[data-vitt-theme]` / `[data-vitt-accent]` |
| `tokens/type.css` | the type scale and the `.vitt-num` tabular-figures class |
| `tokens/space.css` | spacing and radii |

Generated from `composeApp/src/commonMain/kotlin/ie/shoonya/vitt/ui/theme/` in
the VITT repo, which is the sole definition of these values.
