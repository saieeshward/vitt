...
- Cairn has **no server** and **no Cairn account**. There is no central store to
  breach.
...
- The `drive.file` scope means a compromised token grants access only to the
  spreadsheet Cairn created, not the user's wider Drive.
- Financial data at rest relies on platform encryption — iOS Data Protection and
  ...
  that should change.
- Anything a user can see in their own spreadsheet, they can also edit. Cairn
